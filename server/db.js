...
// takes only a single argument for the model name
...
// returns the instance with valid inputs and null with an invalid ID
...
// does not check to make sure that valid inputs are supplied, so you will have to add those checks to your routes if necessary
// will return the newly-created instance from the database
// validates the schema of the instance to create and throws an error if it is invalid
...
// validates the schema of the updated instance and throws an error if it is invalid
...
// you will only need to use this function for an /api/meetings route.
